...
const markdownComponents: Components = {
  ...
  h1({ children }) {
    return <h1 className="mt-5 mb-2 first:mt-0 text-lg leading-8 font-semibold">{children}</h1>;
  },
  h2({ children }) {
    return <h2 className="mt-5 mb-2 first:mt-0 text-base leading-7 font-semibold">{children}</h2>;
  },
  h3({ children }) {
    return <h3 className="mt-4 mb-1.5 first:mt-0 text-sm leading-7 font-semibold">{children}</h3>;
  },
  hr() {
  ...
  pre({ children }) {
    return (
      <pre className="my-3 overflow-x-auto rounded-lg bg-muted p-3 text-[13px] leading-6 text-foreground [&_code]:bg-transparent [&_code]:p-0 [&_code]:text-[13px] [&_code]:text-inherit">
        {children}
      </pre>
    );
  },
